...
export const Admin: React.FC = () => {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-gray-50 pt-24 md:ml-64 px-4 md:px-8 flex items-center justify-center text-gray-500">
                正在加载...
            </div>
        );
    }
    ...
    if (!user || user.role !== 'admin') {
        return (
            <div className="min-h-screen bg-gray-50 pt-24 md:ml-64 px-4 md:px-8 flex flex-col items-center justify-center text-center text-gray-600">
    ...
    return (
        <div className="min-h-screen bg-gray-50 pt-24 md:ml-64 px-4 md:px-8 pb-12">
                ...
                <div className="bg-white border border-gray-100 rounded-xl shadow-sm p-6">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-lg font-semibold text-gray-800">AI 模型配置</h2>
                        </div>
                        <button
                    ...
                    <div className="space-y-4">
                        {aiConfigs.length === 0 ? (
                            ...
                            aiConfigs.map((config) => (
                                <div key={config.id} className={`p-4 rounded-lg border transition-all ${config.is_active ? 'border-indigo-500 bg-indigo-50 ring-1 ring-indigo-500' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                            <h3 className="font-bold text-gray-900">{editingConfig ? '编辑模型配置' : '添加模型配置'}</h3>
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 mb-4">
                                ...
                                <div className="flex gap-2">
                                    <textarea
                                        ...
                                        className="flex-1 px-3 py-2 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all h-16 resize-none font-mono"
                                        onChange={(e) => {
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">模型名称 (Model ID)</label>
                                <input
                                    type="text"
                        ...
                        <div className="px-6 py-4 bg-gray-50 flex justify-end gap-3">
                            <div className="flex-1 flex items-center gap-2">
                                <button
                                    ...
                                    className={`px-4 py-2 border border-gray-300 text-gray-700 font-medium rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2 ${testingConnection ? 'opacity-50 cursor-not-allowed' : ''}`}
                                >
                                    {testingConnection ? (
                                        <>
                                ...
                                {testResult && (
                                    <span className={`text-xs font-medium ${testResult.status === 'success' ? 'text-green-600' : 'text-red-600'}`}>
            ...
            {showLogsModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh]">
                        <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-gray-50">
                            <h3 className="font-bold text-gray-900">使用日志</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            {logs.length === 0 ? (
                                <div className="text-center text-gray-500 py-8">暂无使用记录</div>
                            ) : (
