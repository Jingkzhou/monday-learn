...
export const TestMode: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-[#0a092d] pb-20">
            {/* Header */}
            <div className="sticky top-0 z-10 bg-white dark:bg-[#15143c] border-b border-gray-200 dark:border-white/10 px-4 py-3 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                    <div className="flex flex-col">
                        <div className="text-primary text-xs font-bold uppercase tracking-wider">测试</div>
                        <h1 className="font-bold text-gray-900 dark:text-white text-lg hidden sm:block">{MOCK_SET.title}</h1>
                    </div>
                </div>
                <div className="flex items-center gap-2 sm:gap-4">
                    <div className="relative" ref={dropdownRef}>
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-[#15143c] border border-gray-200 dark:border-white/10 hover:bg-gray-50 dark:hover:bg-white/5 hover:border-gray-300 dark:hover:border-white/20 rounded-md text-sm font-medium text-gray-700 dark:text-gray-300 transition-all shadow-sm"
                        >
                            <Filter className="w-4 h-4 text-gray-500 dark:text-gray-400" />
                            <span className="hidden sm:inline">题型选择</span>
                            <ChevronDown className={`w-3 h-3 text-gray-400 dark:text-gray-500 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>

                        {isDropdownOpen && (
                            <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-[#15143c] rounded-lg shadow-xl border border-gray-100 dark:border-white/10 py-2 z-20 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    <button className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full" title="设置">
                        <Settings className="w-6 h-6" />
                    </button>
                    <button onClick={() => navigate(`/set/${id}`)} className="p-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full" title="关闭">
                        <X className="w-6 h-6" />
                    </button>
                </div>
            </div>
            ...
            {
                isSubmitted && (
                    <div className="max-w-3xl mx-auto mt-8 p-8 bg-white dark:bg-[#15143c] rounded-xl border border-gray-200 dark:border-white/10 shadow-lg text-center mx-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        <div className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">你的分数</div>
                        <div className="text-6xl font-black text-primary mb-4">{score}%</div>
                        <div className="text-gray-700 dark:text-gray-300 font-medium mb-8 text-lg">
                            {score === 100 ? "太棒了！满分！🎉" :
                                score >= 80 ? "做得好！表现不错。🌟" :
                                    "继续练习！你会成功的。💪"}
                        </div>
                ...
                {questions.some(q => q.type === 'written') && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500">
                        ...
                        {questions.filter(q => q.type === 'written').map((q, i) => {
                            ...
                            return (
                                <div key={q.id} className={`bg-white dark:bg-[#15143c] p-6 rounded-lg border shadow-sm transition-colors ${isSubmitted ? (isCorrect ? 'border-emerald-200 dark:border-emerald-900/50 bg-emerald-50/30 dark:bg-emerald-900/10' : 'border-red-200 dark:border-red-900/50 bg-red-50/30 dark:bg-red-900/10') : 'border-gray-200 dark:border-white/10'}`}>
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <div className="text-sm text-gray-400 font-bold mb-2">定义</div>
                                            <div className="text-lg text-gray-800 dark:text-gray-100">{q.prompt}</div>
                                        </div>
                                        ...
                                        {!isSubmitted && (
                                            <div className="flex bg-gray-100 dark:bg-gray-800 rounded-lg p-1">
                                                <button
                                                    ...
                                                    className={`p-1.5 rounded-md transition-colors ${mode === 'keyboard' ? 'bg-white dark:bg-gray-700 text-primary shadow-sm' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
                                                    title="键盘输入"
                                                >
                                                ...
                                                <button
                                                    ...
                                                    className={`p-1.5 rounded-md transition-colors ${mode === 'handwriting' ? 'bg-white dark:bg-gray-700 text-primary shadow-sm' : 'text-gray-400 hover:text-gray-600 dark:hover:text-gray-300'}`}
                                                    title="手写输入"
                                                >
                                        ...
                                        {mode === 'keyboard' || isSubmitted ? (
                                            <>
                                                <input
                                                    type="text"
                                                    placeholder="输入答案..."
                                                    className={`w-full p-3 border-b-2 outline-none bg-gray-50 dark:bg-gray-800 focus:bg-white dark:focus:bg-gray-900 transition-colors rounded-t-md ${isSubmitted
                                                        ? isCorrect
                                                            ? "border-emerald-500 text-emerald-700 dark:text-emerald-400 font-bold bg-white dark:bg-emerald-900/10"
                                                            : "border-red-500 text-red-700 dark:text-red-400 bg-white dark:bg-red-900/10"
                                                        : "border-gray-300 dark:border-gray-600 focus:border-primary text-gray-900 dark:text-white"
                                                        }`}
                                                    value={answers[q.id] || ''}
                ...
                {questions.some(q => q.type === 'multiple-choice') && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500 delay-100">
                        ...
                        {questions.filter(q => q.type === 'multiple-choice').map((q, i) => {
                            ...
                            return (
                                <div key={q.id} className={`bg-white dark:bg-[#15143c] p-6 rounded-lg border shadow-sm ${isSubmitted ? (isCorrect ? 'border-emerald-200 dark:border-emerald-900/50 bg-emerald-50/30 dark:bg-emerald-900/10' : 'border-red-200 dark:border-red-900/50 bg-red-50/30 dark:bg-red-900/10') : 'border-gray-200 dark:border-white/10'}`}>
                                    <div className="text-sm text-gray-400 font-bold mb-2">术语</div>
                                    <div className="text-lg text-gray-800 dark:text-gray-100 mb-4 font-serif">{q.prompt}</div>
                                        ...
                                        {q.options?.map((opt, oIdx) => {
                                            ...
                                            if (isSubmitted) {
                                                if (opt === q.correctAnswer) {
                                                    style = "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-800 dark:text-emerald-300 font-bold shadow-sm";
                                                    icon = <CheckCircle className="w-5 h-5 text-emerald-600" />;
                                                } else if (userAns === opt) {
                                                    style = "border-red-500 bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300";
                                                    icon = <XCircle className="w-5 h-5 text-red-500" />;
                                                } else {
                                                    style = "border-gray-100 dark:border-white/5 opacity-40 text-gray-600 dark:text-gray-400";
                                                }
                                            } else if (userAns === opt) {
                                                style = "border-primary bg-indigo-50 dark:bg-indigo-900/30 text-primary font-semibold shadow-sm ring-1 ring-primary";
                                            } else {
                                                style = "border-gray-200 dark:border-white/10 hover:border-primary hover:bg-gray-50 dark:hover:bg-white/5 text-gray-600 dark:text-gray-300";
                                            }
                ...
                {questions.some(q => q.type === 'true-false') && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-500 delay-200">
                        ...
                        {questions.filter(q => q.type === 'true-false').map((q, i) => {
                            ...
                            return (
                                <div key={q.id} className={`bg-white dark:bg-[#15143c] p-6 rounded-lg border shadow-sm ${isSubmitted ? (isQuestionCorrect ? 'border-emerald-200 dark:border-emerald-900/50 bg-emerald-50/30 dark:bg-emerald-900/10' : 'border-red-200 dark:border-red-900/50 bg-red-50/30 dark:bg-red-900/10') : 'border-gray-200 dark:border-white/10'}`}>
                                    <div className="flex flex-col md:flex-row md:gap-8 gap-4">
                                        <div className="flex-1 border-b md:border-b-0 md:border-r border-gray-100 dark:border-white/10 pb-4 md:pb-0 pr-0 md:pr-4">
                                            <div className="text-sm text-gray-400 font-bold mb-1">术语</div>
                                            <div className="text-lg text-gray-800 dark:text-gray-100 font-serif">{q.prompt}</div>
                                        </div>
                                        <div className="flex-1">
                                            <div className="text-sm text-gray-400 font-bold mb-1">定义</div>
                                            <div className="text-lg text-gray-800 dark:text-gray-100">{q.tfstatement}</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex gap-4 mt-6">
                                        {['true', 'false'].map((opt) => {
                                            ...
                                            if (isSubmitted) {
                                                if (opt === q.correctAnswer) {
                                                    style = "border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-300 font-bold shadow-sm";
                                                    icon = <CheckCircle className="w-5 h-5 text-emerald-600" />;
                                                }
                                                else if (userAns === opt) {
                                                ...
                                                style = "border-gray-200 dark:border-white/10 hover:border-gray-400 dark:hover:border-white/30 text-gray-600 dark:text-gray-300";
                                            }
