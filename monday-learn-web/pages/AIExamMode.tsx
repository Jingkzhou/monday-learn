...
export const AIExamMode: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 print:bg-white font-serif md:font-sans">
      {/* Navigation Header - Hidden when printing */}
      <div className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between sticky top-0 z-20 print:hidden">
          ...
          {status === 'complete' && (
            <>
               <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 text-sm font-medium border rounded-lg transition-colors ${showAnswers ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-gray-300 text-gray-600 hover:bg-gray-50'}`}
              >
              ...
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-gray-600 bg-gray-50 border border-gray-300 rounded-lg hover:bg-gray-100 transition-colors"
              >
      ...
      <div className="max-w-[210mm] mx-auto my-8 print:my-0 print:w-full print:max-w-none">
        ...
        {status === 'generating' && (
          <div className="flex flex-col items-center justify-center h-[60vh] bg-white rounded-xl shadow-sm p-8 animate-in fade-in duration-500">
            <div className="w-16 h-16 border-4 border-primary border-t-transparent rounded-full animate-spin mb-6"></div>
            <h2 className="text-xl font-bold text-gray-800 mb-2">正在为您生成试卷...</h2>
            <p className="text-gray-500 max-w-md text-center">AI 正在分析学习集内容、判断难度等级，并设计变形题目，请稍候。</p>
          </div>
        )}

        {status === 'error' && (
          <div className="flex flex-col items-center justify-center h-[60vh] bg-white rounded-xl shadow-sm p-8">
             <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mb-4">
                <AlertTriangle className="w-8 h-8" />
             </div>
             <h2 className="text-xl font-bold text-gray-800 mb-2">生成失败</h2>
             <p className="text-gray-500 mb-6 text-center">AI 服务暂时无法响应，请检查网络连接或重试。</p>
             <button 
                onClick={generateExam}
        ...
        {status === 'complete' && examData && (
          <div className="bg-white shadow-lg print:shadow-none p-[15mm] md:p-[20mm] min-h-[297mm] relative animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Exam Header */}
            <div className="text-center border-b-2 border-gray-800 pb-6 mb-8">
              <h1 className="text-3xl font-serif font-bold mb-3 text-black tracking-tight">{examData.title}</h1>
              <div className="flex items-center justify-center gap-4 text-gray-600 mb-6 font-medium">
                <span className="bg-gray-100 print:bg-transparent print:border print:border-gray-300 px-3 py-1 rounded-full text-sm text-gray-800">{examData.subject}</span>
                <span>•</span>
              ...
              <div className="flex justify-between text-sm font-medium mt-8 px-4 md:px-12">
                <div className="flex gap-2 items-end">
                    ...
                    <div className="border-b border-black w-24 md:w-32"></div>
                </div>
                <div className="flex gap-2 items-end">
                ...
                <div className="flex gap-2 items-end">
                    ...
                    <div className="border-b border-black w-16 md:w-24"></div>
                </div>
              </div>
            </div>
                ...
                <div key={index} className="mb-8 break-inside-avoid">
                  <div className="mb-4">
                    <h3 className="text-lg font-bold text-black mb-1 flex items-center gap-2">
                      <span className="bg-black text-white w-6 h-6 flex items-center justify-center rounded text-sm print:text-black print:bg-transparent print:border print:border-black">{["I", "II", "III", "IV", "V"][index]}</span>
                      {section.title}
                    </h3>
                    <p className="text-sm text-gray-500 italic ml-8">{section.description}</p>
                  </div>

                  <div className="space-y-6 pl-2">
                    {section.questions.map((q, qIndex) => (
                      <div key={q.id} className="break-inside-avoid relative pl-6">
                        <span className="absolute left-0 top-0 font-serif font-bold text-gray-800">{qIndex + 1}.</span>
                        <div className="font-medium text-gray-900 leading-relaxed mb-2">
                          {q.text}
                          {q.type === 'true_false' && (
                            <span className="float-right print:block text-gray-400 font-normal tracking-widest">( &nbsp;&nbsp;T / F&nbsp;&nbsp; )</span>
                          )}
                        </div>
                        ...
                        {showAnswers && q.correctAnswer && (
                            <div className="mt-2 p-2 bg-emerald-50 border border-emerald-100 rounded text-sm text-emerald-800 print:hidden">
            ...
            {/* Footer */}
            <div className="mt-16 text-center text-[10px] uppercase tracking-widest text-gray-300 border-t border-gray-100 pt-6 print:hidden">
